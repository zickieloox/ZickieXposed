...
if [ ! -f "system/xposed.prop" ]; then
  echo "! Failed: Extracted file system/xposed.prop not found! - file_error"
  exit 1
fi
...
if [ ! -f '/system/build.prop' ]; then
  echo "! Failed: /system could not be mounted! - mount_error"
  exit 1
fi
...
if [ -z $XVALID ]; then
  ...
  echo "! for your platform/ROM! - invalid_error"
  exit 1
fi
